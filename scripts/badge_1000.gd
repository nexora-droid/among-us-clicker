extends Button

@onready var score: Label = $"../../../../Score"
@onready var badge_title: Label = $BadgeTitle
@onready var badge_detail: Label = $BadgeDetail
@onready var claim_1_b_3: Panel = $Claim1_B3
@onready var claim_2_b_3: Panel = $Claim2_B3
@onready var claim_prompt_b_3: Label = $ClaimPrompt_B3
func _ready() -> void:
	disabled = true
	badge_detail.hide()
	badge_title.hide()
	claim_1_b_3.hide()
	claim_2_b_3.hide()
	claim_prompt_b_3.hide()
	score.connect("score_1000", Callable(self, "_on_score_reach_1000"))
	score.connect("badge3_claimed", Callable(self, "_on_badge3_claimed"))
	if score.badges_unlocked[4]:
		_on_score_reach_1000()
	if score.badges_claimed[4]:
		_on_badge3_claimed()
func _on_score_reach_1000() -> void:
	if score.badges_claimed[4]:
		disabled = true
		badge_title.show()
		badge_detail.show()
		return
	disabled = false
	badge_title.show()
	badge_detail.show()
	claim_1_b_3.show()
	claim_2_b_3.show()
	claim_prompt_b_3.show()
func _on_badge3_claimed() -> void:
	disabled = true
	add_theme_stylebox_override("disabled", load("res://assets/button_styles/mid_badge.tres"))
	badge_title.position = Vector2(274, 23)
	badge_detail.position = Vector2(310, 67)
	custom_minimum_size = Vector2(878, 115)
	
